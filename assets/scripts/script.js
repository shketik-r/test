...
const copyBtns = document.querySelectorAll(".copy-to-clipboard-btn");

copyBtns.forEach((btn) => btn.addEventListener("click", copyContent));
copyBtns.forEach((btn) => btn.addEventListener("mouseleave", onCopyBtnBlur));

function onCopyBtnBlur(e) {
  const clickedBtn = e.target;
  const clickedBtnTooltip = clickedBtn.firstElementChild;

  clickedBtnTooltip.innerText = "Копировать";
}
...
async function copyContent(e) {
  try {
    const clickedBtn = e.target;
    const clickedBtnTooltip = clickedBtn.firstElementChild;
    const textToCopy = clickedBtn.innerText.split("\n")[0];

    await navigator.clipboard.writeText(textToCopy);
    clickedBtnTooltip.innerText = "Координаты\xa0скопированы\xa0✓";
  } catch (err) {
  ...
  input.addEventListener("input", (e) => {
    ...
    if (typeof value === "string") {
      if (value.startsWith("+375") && value !== "+375") {
        const belarusianPhoneGroups = value
          .replace(/\D/g, "")
          .match(/(\d{0,3})(\d{0,2})(\d{0,3})(\d{0,2})(\d{0,2})/);

        if (belarusianPhoneGroups) {
          e.target.value =
            (belarusianPhoneGroups[1] ? `+${belarusianPhoneGroups[1]}` : "") +
            (belarusianPhoneGroups[2] ? ` (${belarusianPhoneGroups[2]}` : "") +
            (belarusianPhoneGroups[3] ? `) ${belarusianPhoneGroups[3]}` : "") +
            (belarusianPhoneGroups[4] ? `-${belarusianPhoneGroups[4]}` : "") +
            (belarusianPhoneGroups[5] ? `-${belarusianPhoneGroups[5]}` : "");
        }
      } else if (value.startsWith("+7") && value !== "+7") {
        const russianPhoneGroups = value
          .replace(/\D/g, "")
          .match(/(\d{0,1})(\d{0,3})(\d{0,3})(\d{0,2})(\d{0,2})/);

        if (russianPhoneGroups) {
          e.target.value =
            (russianPhoneGroups[1] ? `+${russianPhoneGroups[1]}` : "") +
            (russianPhoneGroups[2] ? ` (${russianPhoneGroups[2]}` : "") +
            (russianPhoneGroups[3] ? `) ${russianPhoneGroups[3]}` : "") +
            (russianPhoneGroups[4] ? `-${russianPhoneGroups[4]}` : "") +
            (russianPhoneGroups[5] ? `-${russianPhoneGroups[5]}` : "");
        }
      } else {
        if (!value.match(/^\+?[\s\d\(\)\-]{0,20}$/g)) e.target.value = value.slice(0, -1);
      }
    }
  })
);
...
dateInputs.forEach((input) => input.setAttribute("min", new Date().toISOString().split("T")[0]));

const cashlessPaymentCheckbox = document.getElementById("payment-cashless");
const cashPaymentCheckbox = document.getElementById("payment-cash");
const accountDetailsInput = document.querySelector(".account-details-input-wrapper");

cashlessPaymentCheckbox?.addEventListener("input", () => {
  accountDetailsInput.classList.add("hidden");
});

cashPaymentCheckbox?.addEventListener("input", () => {
  accountDetailsInput.classList.remove("hidden");
});
    ...
    jQuery(t)
      ...
      .on("click", function (e) {
        ...
        if (jtarget.length > 0) {
          ...
          if (target != "#" && target.length > 2 && jQuery(this).closest("header").length)
...
function isEmpty(obj) {
  if (
    ...
    obj !== null &&
    (obj.length > 0 || typeof obj == "number" || typeof obj.length == "undefined") &&
    obj !== "undefined"
  )
    return false;
  else return true;
...
function isScrollView(t) {
  ...
  if (e < tp + 300 || jQuery(window).scrollTop() + jQuery(window).height() == jQuery(document).height())
    return true;
  else return false;
}
      ...
      function r(t, u, f) {
        ...
        if (arguments.length > 1) {
          ...
          typeof f.expires == "number" &&
            ((s = new Date()), s.setMilliseconds(s.getMilliseconds() + f.expires * 864e5), (f.expires = s));
          try {
            o = JSON.stringify(u);
            /^[\{\[]/.test(o) && (u = o);
          } catch (y) {}
        ...
        t || (o = {});
        for (
          var l = document.cookie ? document.cookie.split("; ") : [], a = /(%[0-9A-Z]{2})+/g, h = 0;
          h < l.length;
          h++
        ) {
          var v = l[h].split("="),
  ...
  $.fn.toggleClick = function (n) {
    var t = arguments,
      ...
      u = function (r) {
        ...
        return (
          $._data(this, "lastToggle" + n.guid, u + 1), r.preventDefault(), t[u].apply(this, arguments) || !1
        );
      };
      ...
      start: function () {
        ...
        if ($(obj).hasClass("thumb") || $(obj).hasClass("nav-middle"))
          $(obj)
            ...
            .css("top", $(obj).find(".slides > li img")[0].clientHeight / 2 + "px");
        $(obj)
  ...
  $(document).ready(function () {
    ...
    if ($("#wpadminbar").length) {
      ...
      if ($("header").hasClass("side-menu-header"))
        $("header .side-menu-fixed,header .navbar-fixed-top").css("margin-top", "32px");
    }
    cache = $("header a");
    $(cache).each(function () {
      if ($(this).attr("href") == window.location.href) {
        if ($(this).closest(".dropdown-menu").length) {
          $(this)
            .closest(".dropdown.multi-level:not(.dropdown-submenu),.dropdown.mega-dropdown")
            .addClass("active");
        } else {
